...
function tailorManifest(base, browser) {
  ...
  if (browser === "firefox") {
    ...
    m.browser_specific_settings.gecko.data_collection_permissions = { required: ["none"] };
...
function build() {
  ...
  function validateCssDualWriteMarkers() {
    // ADR-0011: validator reads concatenated source files, not the ntp.css build artifact.
    // ntp.css is gitignored and generated by buildNtpCss(). Reading the source files
    // directly ensures the validator works even before the build runs.
    const srcDir = path.join(ROOT, "ntp");
    const srcFiles = ["base.css", "settings.css", "onboarding.css", "conn.css"];
    const css = srcFiles.map(f => {
      const fp = path.join(srcDir, f);
      if (!fs.existsSync(fp)) { console.warn("A8: source file missing: " + f); return ""; }
