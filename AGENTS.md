...
| BX-MANIFEST-004 | MUST | After any change to source `manifest.json` OR `ntp/` OR `background.js`, `build.mjs` MUST be re-run; otherwise `dist/` will be stale vs the source. Run: `node .github/scripts/build.mjs` (or `node scripts/build-release.js` which wraps it). **Cross-platform wrappers**: `tools/build.ps1` (Windows PowerShell) and `tools/build.sh` (Linux/macOS bash) both call `build.mjs` — use whichever matches your shell. Stale `dist/` is the root cause of user-facing "旧版 UI" symptoms when loading the rebuilt package. |
