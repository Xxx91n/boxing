...
5. **Every derived index is a liability.** When adding a new data structure (connById, boxConnIdx, groupStar, layout.groups), EVERY mutation path (addConnection, removeConnection, _execDeleteLargeBox, _execDeleteSmallBox, toggleStarMark) must update ALL of them. Missing one = bug. See [docs/architecture-audit.md](docs/architecture-audit.md) Part 1.
...
7. **`layout.groups` is a compat shim, NOT source of truth.** It is rebuilt by `ensureGroups()` from `box.isParent` + DSU. Never trust `layout.groups` without a `dsuRebuildFromConnections()` first. Future goal: stop persisting it entirely (see architecture-audit.md Part 2, Issue 1).
...
9. **The "fix one bug, introduce another" pattern exists because mutation paths are scattered.** Each of removeConnection, _execDeleteLargeBox, _execDeleteSmallBox, toggleStarMark has its own cleanup sequence. A single unified `mutateLayout(operation)` API would eliminate this. See architecture-audit.md Part 3 for the coupling map.
...
11. **DSU rebuild is O(n) — gate it behind a dirty flag.** `__dsuBuilt` prevents redundant rebuilds within a tick. Future: add `__dsuDirty` set by mutations, checked by `getGroupByParent` before returning. See architecture-audit.md Part 4.3.
12. **`moveGroupTogether` is O(m*n) per frame — the main performance hotspot.** For each group member, `elasticSnap` scans all non-group boxes. At >10 boxes this causes frame drops. Future: pre-compute a spatial index (grid hash) at drag-start. See architecture-audit.md Part 4.2, Hotspot 1.
...
| BX-MANIFEST-002 | MUST | Two build scripts produce browser-tailored rebuilds and MUST stay in lockstep on the Tailor logic: `scripts/build-release.js` (local — thin wrapper that calls `.github/scripts/build.mjs`) — outputs `dist/boxing-{chrome,firefox}/release/{chrome,firefox}/`. `.github/scripts/build.mjs` (CI + local — outputs `dist/boxing-{chrome,firefox}/` + nested `release/{chrome,firefox}/` with 3 artifacts each: unpacked `boxing/`, `.zip`, `.crx`/`.xpi`). Both scripts share the same build logic — `build-release.js` just spawns `build.mjs`. |
