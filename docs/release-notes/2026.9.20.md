...
### 内部
- **写路径一致性**：新增书签（addBookmark）与删除/重排同走 `commit(op)` 统一变更入口
- **layout 旁路豁免可审计**：9 条改写豁免全部具名 + 到期日 + 工单路径；删除类保持零豁免
- **flaky 豁免清零**：3 条过期 active 按规则处置（2 撤账 + 1 expired-handled），禁止续期
- **Pages 发行放行**：github-pages environment 允许 `v*` tag 触发 deploy
