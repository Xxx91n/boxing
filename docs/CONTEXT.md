...
- **Wave8 grill (2026-09-12)** — Release candidate version is **2026.9.12** rebuilt from main including zero-flash; G-B must run on that new unpacked artifact (not the 2026.9.13 rehearsal tree). G4/G5 baseline is **v2026.9.11** only. G-A residual reds are bucketed **N (never-quarantine) / B (broken) / F (flaky)** per waiver-ledger rules; N/B may not be waived. Plan lanes: T-GA1–5 (G-A), R1–R8 (锐评 reopen incl. conflict-copy UI, CRED per-install, WebDAV opt-in, perf/search debounce, popup, docs productization, ticket hygiene), human items B41/B42/B43. Wave7 A-021..A-024 deferred items are explicitly reopened for Wave8 (not silently rewritten).
- **Wave8 settle (2026-09-12)** — Tickets 70–86 + 79R/81R/81R2 landed: G-A set-diff (H1 否证/H3 成立), N-bucket gate4/state-sync/dr-export 修绿, B-bucket unload-flush + persistView→saveLayout, conflict-copy readout UI, CRED per-install PIK via storage credKeyGet/Set narrow port (gate2 green), WebDAV private-host opt-in (webdavAllowPrivateHost), search debounce 120ms, popup terracotta→accent, docs START-HERE + pointer check, ticket hygiene 61/63/64/66. G-B/慢放 by user-forced D-009/A-038 (no artifacts; NOT auditable G-B). Issue #9 closed as process-only (A-039) while G-A CI still pending. Release gate unchanged (G-A∧G-B∧G-C); tag still forbidden.
- **Wave9 grill (2026-09-13)** — Target version **2026.9.15**; hold **2026.9.12** (no hotfix). Store AMO+Edge visible for 2026.9.12 (user-declared). G-A definitive on `b4f3df2` run 34749813393: data-golden green, three-OS tests red (R1 star-sync B-bucket / R2 empty-state pending / R3 auto-expand F) → **G-A not satisfied**. G-B evidence form = **user-declared pass** (version+date); agents must not sign G-B. Linear append-only on `origin/main` from `b4f3df2` (no new root). Scope: R1–R3, B54–B56, B55 full merge implementation, paper debt (ADR-0017 revision, release-notes, badges, AGENTS portable paths, history declaration). Ledger: `.scratch/wave9-postrelease-grill/decision-ledger.md` D-001..D-010.
- **Wave9 merge (2026-09-13 · ticket 91 / A-045 / B55)** — WebDAV concurrent merge is now an id-level three-way: client-local `boxingLayout.syncBase` slot (storage-facade-owned `getSyncBase`/`setSyncBase`, never uploaded, Joplin base_* model) + `utils.mergeLayoutThreeWay` merging small boxes per id — one-sided edits auto-merge, pure deletions are honored, delete-vs-edit keeps content, and same-id child divergence archives parent-wrapped `webdav-child-conflict` copies into the ticket-79 readout. No-base path = additive two-way (never worse; the B55 silent-swallow is gone). The >3-field heuristic survives only for boxes without a base record. E2E: `test/tests/boxing-merge-three-way.spec.ts`.
- **Wave9.15 closeout audit (2026-09-14)** — Tickets 96–106 implemented (brain-reviewed). Hard acceptance local green (pretest 6-gate + contrast/docs-link/waiver). Packaging fix PACK-01: release zip excludes `.scratch`. Version surface 2026.9.15. Release G-A must re-run on post-land tip; G-B remains user-declared; tag/store still require a separate order.
- **Wave9.15 grill settle (2026-09-14)** — Release closeout grill finalized (D-001..D-009 in `.scratch/wave9-915-release-grill/decision-ledger.md`). Combo: A release path + B complete all debt (docs-gov, rui6 Release status, rui8 freeze comments, B63-B70). Intermediate G-A = run 34808080000 @ 9fa4666c; **release G-A must re-run on post-B-track tip**. G-B user-declared only; tag/store separate order. Mixed tickets GH+scratch.
...
- **Wave9.20 version surface (2026-09-15 · ticket 116 / A-071)** — version surface advanced to **2026.9.20**: manifest `version`+`version_name`, package.json/lock, `ntp/index.html` static footer, AGENTS.md, CHANGELOG `[2026.9.20]`, `docs/release-notes/2026.9.20.md` (build.yml `body_path` SSOT), publishing-guide/store-publishing-plan/STORE-COPY/gb-dryrun retargeted (G-B artifact `rel-2026.9.20`, upgrade baseline v2026.9.15). Store truth stays 2026.9.15 (AMO+Edge published, user-declared); **no releasability claim before G-A∧G-B∧G-C** (ADR-0017); `node scripts/calver-guard.mjs` exit 0. Report: `.scratch/architecture-recovery/reports/116-report.md`.

### Mutation API (ADR-0007)
- **commit(op, payload, opts)** — single mutation entry (tldraw Store pattern). Ops: `addConn`, `removeConn`, `toggleStar`, `deleteLargeBox`, `deleteSmallBox`, `deleteBookmark` (ticket 107: single-bookmark delete; tombstones the bm id), `reorderBookmarks` (ticket 108: drag reorder of a small box bookmarks array; net-neutral so it tombstones nothing), `applyExternal`. Owns tombstones, DSU dirty, viewState clear, optional save/render.
...
- **UI-AUDIT.md**: [docs/agents/ui-audit.md](agents/ui-audit.md) — Phase 7 aesthetic audit: 7-dimension findings + remediations, token validation checklist (ADR-0008 Phase 7)
...
### Data Resilience & Release Gate (ADR-0009/0016/0017 · spec wave5 票 40-47)
- **snap.v1 split-key snapshots** — each snapshot is its own `chrome.storage.local` key `snap.v1.<ts>`, with a lightweight index key `snap.v1.index` storing `{ts, schemaVersion, size}`. Replaces the `boxingSnapshots[]` monolith (Sidebery #1057 large-key loss lesson, ticket 41). Owned exclusively by the `ntp/storage.js` facade.
- **Time Machine rotation** — tiered retention over the snap.v1 set: hourly within 24h, daily within 30d, weekly beyond; `RAW_KEEP_FLOOR=10` minimum keep + 8MB total-byte LRU cap (ticket 41).
- **pre-update snapshot** — copy-on-write before any schema migration: `background.js takePreUpdateSnapshot()` (SW self-contained, first side effect of the `onInstalled(reason=update)` branch) + NTP-side `ensurePreUpdateSnapshot()` (update signal + `needsMigration(raw)` → snapshot the un-migrated payload before `loadLayout()` migrates) (ticket 42/42R).
- **needsMigration(raw)** — whitelist-style "migration pending" detector (v<3 / boxes non-array / settings missing / groups residue / `__groupsMigrated` unsealed / connections without `props`); a false positive only costs one extra snapshot (COW: rather too many than too few).
...
- **Wave9.15 release (2026-09-14)** — User G-B pass declared for 2026.9.15. GitHub Release [v2026.9.15](https://github.com/Xxx91n/boxing/releases/tag/v2026.9.15) published (chrome/firefox/source/SHA256SUMS) at `cc30edcc`. G-C three URLs 200. Store upload is user-manual; not agent-automated.
- **Wave9.20 grill settle (2026-09-15)** — Target **2026.9.20**; linear append on `a1acaaac` (no new root). P0 bookmark-delete resurrection: architectural `deleteBookmark` via `commit` + static gate against layout bypasses + never-quarantine regression (D-002). G-C upgraded: three URL 200 **and** `demo/version.json` == latest tag + deploy-tail verify; human checklist retained (D-003). Backlog B71–B75+B77 in scope; B76 out; B78 stays closed (D-004). release-status single status block; waiver clear-down to 0 active F before G-A (D-005). G-B user-declared; tag/store separate order (D-006). Ledger: `.scratch/wave9-920-grill/decision-ledger.md` D-001..D-008.
- **Wave9.20 settle (2026-09-15)** — Tickets 107–116 implemented (W1/W2 brain-reviewed). P0 bookmark delete via `commit('deleteBookmark')` + tombstones + never-quarantine regression (A-062). Static `layout-bypass-guard` + `reorderBookmarks` op (A-063). G-C freshness: `version.json` + deploy-tail verify + ADR-0017 G-C revision (A-064). B71–B77 closed except B76 (store listing out of scope). Version surface **2026.9.20** (A-071). G-B user-declared pass 2026-09-15 on 2026.9.20 zips. Linear tip on `a1acaaac` (A-P01). Named residuals: N-108-01 (add bypass), F-113-01 (demo render lag), environment tag allow (user).
- **Wave9.20 followup grill settle (2026-09-22)** — 目标=全量可解项本次收口（D-001）。责任：C08 tag/商店=用户，其余含 C04=agent；G-B 不代签，发行件变更则旧声明对新产物失效（D-002/D-004）。过期 flaky 豁免：先已修后撤账，未修书面退役，禁止续期（D-003）。layout-bypass 终点=分层：删除/墓碑类零豁免走 commit(op)；非集合改写可留具名豁免但须到期；棘轮只减不增（D-005）。票务=.scratch 单票无 GH Issue（D-006）。Ledger: `.scratch/wave9-920-followup/decision-ledger.md` D-001..D-007。
