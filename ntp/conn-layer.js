...
export function initConnFacade(deps) {
  ...
  export function pruneConnArrays(onSave) {
    ...
    const isValidKey = k => validKeys.has(k) || rawIds.has(k);   // tiered or legacy
      // layout-bypass-allow: conn-layer-prune-validity - save-path sanitize drops unresolvable conns, no user-visible delete (expires:2027-03-31; ticket:108)
    ...
    if (layout.connections.length > MAX_CONNECTIONS) {
      // layout-bypass-allow: conn-layer-max-cap - hard MAX_CONNECTIONS cap enforced on the save path only (expires:2027-03-31; ticket:108)
...
export function ensureGroups() {
  ...
  // layout-bypass-allow: runtime-groups-mirror - ADR-0007 Q1 layout.groups is a runtime mirror never persisted (expires:2027-03-31; ticket:108)
  layout.groups = gs;
  return gs;
}
