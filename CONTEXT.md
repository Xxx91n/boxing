...
- **box drag positioning (BX-EXPLORE-005)**: Box dragging writes `el.style.left` / `el.style.top` only. Never use `transform: translate3d()` during drag (double-offset flash). Implementation carried by the drag handlers in ntp/render.js.
