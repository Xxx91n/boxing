...
|---|---|---|
| SEC-01 | MUST | Mock fallback pi = mock MUST NOT pollute self.chrome / self.browser globals. Keep mock local to avoid breaking other extensions or browser internals in file:// test mode. |
...
| SEC-11 | MUST | nsureHttpsUrl() MUST reject javascript:, data:, bscript: protocols before any URL construction. These dangerous protocols are returned as-is (unmodified) so callers can reject them further. |
| SEC-15 | MUST | 
tp/index.html MUST include a <meta> Content-Security-Policy header: default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self'; img-src 'self' https: data:; connect-src 'self' https:;. This prevents XSS, inline script injection, and unauthorized external resource loading. |

| BX-DEV-111g | MUST | Auto-expand reliability fixes: (1) CSS block integrity restored — visibility:hidden moved inside .box-resize-handle rule block. (2) max-height collapsed clip raised from 54→60px for bar safety margin. (3) setBodyExpandHeight added void el.offsetHeight forced reflow after setting/restoring max-height:none to guarantee scrollHeight measurement reflects actual layout. Debug log added. (4) expandBtn click handlers now call setBodyExpandHeight(el) immediately after adding box--collapsed class — ensures --expand-height is measured when user activates auto-expand. (5) CSS syntax error (orphaned visibility:hidden outside rule) fixed.
