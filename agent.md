...
| BX-DEV-078 | MUST | Pin button (header-pin-btn) must ALWAYS reside on the currently active canvas (canvasContainer or innerCanvas), regardless of headerPinned state. updateAutohideUI() unconditionally moves the button to the active canvas; headerPinned controls only overflow lock and CSS floating class. Called unconditionally from renderCanvas and enterLargeBox (not guarded by !headerPinned). |
| BX-DEV-079 | MUST | URL bookmark opening must respect `layout.settings.urlOpenMode`. Value `newTab` (default): open in new tab via `api.tabs.create({url,active:true})`. Value `currentTab`: use `api.tabs.query` to find the current tab, then `api.tabs.update(tabId,{url})`; if query returns empty, fallback to `api.tabs.create`. Fallback window.open uses `_blank` (newTab) or `_self` (currentTab). Cross-browser (Chrome+Firefox) compatible. |
| BX-DEV-080 | MUST | Settings General tab must include `url-open-mode` select element with data-i18n attributes for urlOpenModeLabel, urlOpenModeNewTab, urlOpenModeCurrentTab, urlOpenModeHint. All 13 locales must have corresponding translations. onchange handler must save to layout.settings.urlOpenMode. |
| BX-DEV-081 | MUST | background.js must listen to `tabs.onUpdated` to detect NTP tab URL changes. When a tab navigates FROM Boxing NTP path to an external URL, read `layout.settings.ntpNavMode` from storage. If `newTab` (default): open destination URL in new tab via `api.tabs.create`, restore NTP via `api.tabs.update`. If `currentTab`: let navigation proceed naturally. Cross-browser (Chrome+Firefox). |
| BX-DEV-082 | MUST | Settings General tab must include `ntp-nav-mode` select with data-i18n attributes. All 13 locales must have ntpNavModeLabel/NewTab/CurrentTab/Hint translations. onchange saves to layout.settings.ntpNavMode. ntpNavSelect element declared after urlOpenSelect. |
