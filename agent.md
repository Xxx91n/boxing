...
| BX-DEV-111 | MUST | Four fixes in v3.7.8: (1) renderCanvas hides canvasEmpty placeholder BEFORE clearing surface to eliminate refresh flash of "no boxes". (2) headerPinned state persisted to layout.settings.headerPinned — survives refresh and new tabs. (3) Canvas pan+zoom position automatically persisted in saveLayout() and via beforeunload listener — refreshed pages restore exact view position rather than resetting to origin. Only brand-new tabs (Navigation Timing type=0) may restore lastLargeBoxId position when rememberLastPos is enabled. (4) Auto-expand uses pure max-height+overflow:hidden+cubic-bezier transition (no scaleY) — entire __body panel pulls down from under the title bar rather than squishing internal text content. Collapsed state: max-height:0, padding:0, opacity:0; hover state: max-height:var(--body-max-height), full padding, opacity:1. |
