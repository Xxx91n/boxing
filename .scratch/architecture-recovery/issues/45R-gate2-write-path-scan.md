...
- [ ] gate2 断言改为写调用/key 扫描；background 读 boxingLayout 不再红
- [ ] 实跑 boxing-data-golden.spec.ts：gate2 passed（其余 gate 行为不回归）
- [ ] migration-golden 仍 4 passed；pretest 双 guard 仍绿
- [ ] 报告追加「返工轮次」：纠正最终 SHA 为真实 tip；写明 gate2 新语义
- [ ] issues/45 AC 表述与新 gate2 一致
