...
- 相邻 `_enc` 面（chromium-extension，16 例）：`boxing-webdav` + `boxing-memory` + `boxing-update-cow-before-migrate` → **16 passed (14.9s)**。其中 `boxing-memory.spec.ts:120` 直接断言 `settings._encWebdavPass` 落库 —— 若本票按「改 _obf」思路重命名该字段，此用例必红；反向实证「保留 legacy 字段名」的判断
