# 回传后复核与写回稿 — 49 G-B / 票 76 重跑（2026.9.12）

> **票 76 重跑口径（2026-09-12）**：本轮 G-B 验收对象 = 2026.9.12（run 34689649760 / `D:/rel-2026.9.12`），双车道勾选单为 `checklist-2026.9.12-{chrome,firefox}.md`。
> GitHub #9 关闭（A-007）归 **票 78**；本文件「四、票 49 关票」节在本轮读作「票 76 关票」，AC 见 `issues/76`。
> 复核与写回仍须在用户完成双浏览器实机勾选并回传证据后执行；代理不得代勾、不得纯自动化宣称（A-009）。
