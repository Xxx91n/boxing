...
|---|---|---|---|---|---|---|
| active | boxing-auto-expand › 'large box with collapseHover=true still expands after visiting and returning' (chromium) | https://github.com/Xxx91n/boxing/actions/runs/34626507101 | expand height 60 (collapsed max-height clamp) vs >80, BEFORE ENTER hover; ubuntu-latest only, macos+windows 同码绿; 票15/票31 已备案 60vs80 hover 抖动 (solo 单跑绿) | 48 | 2026-09-19 (下一条 main 全绿验证 run; 票48 已改轮询硬化, CI 复测绿即撤账) | 票74 逐行复核 2026-09-12: 最新基线 run 34686760142 中本面仍现 60vs80(Expected >80 / Received 60, poll 10s 谓词超时), 但已由 failed 降级为 1 flaky(重试收敛); 仍仅 ubuntu-latest + chromium-extension, macos/windows 零出现 -> 签名匹配, broken/flaky 分界仍成立。main 未全绿, 撤账条件未达成, 维持 active; 到期 2026-09-19 硬到期, 禁止续期。 |
| closed | boxing-zoom-dblclick › 'single click enters; later dblclick inner creates exactly one small box' (firefox) | https://github.com/Xxx91n/boxing/actions/runs/34626507101 | expect.poll inner-visible 5000ms 超时; windows-latest firefox only, 其余 2 OS + chromium 绿; playwright#16095 firefox 有头输入延迟家族 (票01/13 备案) | 48 | 2026-09-19 (同上; 票48 已放宽轮询预算至 15s) | 票74 逐行复核 2026-09-12: run 34686760142 三 OS x 双浏览器零出现(绿); 票48 放宽 expect.poll 预算至 15s 已生效(spec 现 poll timeout=15000) -> 按台账规则撤账 closed。引用 run https://github.com/Xxx91n/boxing/actions/runs/34686760142 |
