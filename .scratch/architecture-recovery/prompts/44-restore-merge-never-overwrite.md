# 窗口启动器 — 44 导入/WebDAV 恢复改为合并+冲突副本，废除静默覆盖

身份: Boxing 子窗口实施代理，只做本票，不跨票。

## 必读（动手前全部读完）
- .scratch/architecture-recovery/handoffs/44-restore-merge-never-overwrite.md
- .scratch/architecture-recovery/issues/44-restore-merge-never-overwrite.md
...
## 完成时
- 遵循 handoff 内的完成定义
- 写报告: .scratch/architecture-recovery/reports/44-restore-merge-never-overwrite-report.md
