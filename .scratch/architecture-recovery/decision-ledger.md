...
| A-049 | 存量 spec 真实 WebDAV 凭据（91R R4：cred-encrypt / dr-export / import-merge / memory / sync 等 + .codex-tmp 副本） | 统一 env 化占位；工作树 grep 0 | 不改写历史；不弱化断言；密码轮换仍属用户 | swept-pending-ci（票95：test/ 22 处 + zip-verify 副本 14 处 env 化，grep 0 / --list 108；历史仍含旧字节；**用户声明已移除/轮换该密码（2026-09-14）**；CI 具名 F-95-CI 待 push） |
