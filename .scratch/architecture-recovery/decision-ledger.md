...
| ID | 问题描述（原文摘录） | 规范化需求 | 显式约束 | 状态 |
|---|---|---|---|---|
| A-062 | D-002 · Q2 调研 · popups.js:189 · 用户日志：P0 书签单条删除旁路 commit：splice 后 fire-and-forget saveLayout、不写墓碑；mergeById 并集复活；盒删除路径已正确 tomb bookmarks | deleteBookmark mutationHandler 走 commit，tombstoneIds:[bmId]；废弃裸 splice；never-quarantine 回归 add→delete→陈旧合并/reload | 禁手术式单行 markDeleted 而不留门禁；禁绕过 commit；数据完整性禁豁免；add/reorder 是否同票实施时裁定；不热修 9.15 | implemented（2026-09-15 · ticket 107 · `ntp/render.js` `mutationHandlers.deleteBookmark` 返回 tombstoneIds:[bmId]；`ntp/popups.js` 删除按钮改走 `commit("deleteBookmark",{largeId,smallId,bmId,index},{save:true})`（裸 splice 已废）；回归 spec `boxing-sync-bookmark-delete-tombstone.spec.ts`（never-quarantine）add→delete→陈旧远端回灌→reload 不复活 + 反证（抹墓碑即复活），双 project 6/6 绿；add/reorder 裁定不同票收编→票 108；docs/CONTEXT.md op 清单同步；报告 reports/107-report.md） |
| A-063 | D-002② · 锐评 Top-5 #5：layout 数据删除/改写可绕过 mutationHandlers（popups add L282/301、reorder L407 等旁路） | 静态门禁：ntp/** 中 layout 集合删除/改写不在 handlers 即 exit 1（白名单注释豁免）；串 pretest 或 test.yml | 白名单须具名；不得静默删检查；与 A-062 同波/紧随 | implemented（2026-09-15 · ticket 108 · `scripts/layout-bypass-guard.mjs` 静态门禁上线：LB-0 allow-region fail-closed（`ntp/render.js` 的 `mutationHandlers` + `commit()` 为唯一免注释合法区）/ LB-1 删除类（splice·pop·shift·unshift）/ LB-2 改写类（重赋值·索引赋值·`Object.assign`·`length=0`·`delete`·sort/reverse/fill/copyWithin）+ 文件内别名 4 轮不动点 / LB-3 孤儿豁免即红（棘轮）；具名豁免语法 `// layout-bypass-allow: <name> - <reason>`（name 必填、reason >= 12 字符，无模块级一把梭豁免）；`--self-test` **18/18 PASS**（含故意旁路 / 别名逃逸 / 无名豁免 / 孤儿豁免 / 标记缺失 fail-closed）；`node scripts/layout-bypass-guard.mjs` exit 0（15 模块 / 8541 行 / 0 违规 / 9 活豁免，每次运行打印活豁免清单）；**唯一真实删除类旁路（`ntp/popups.js` 书签拖拽重排）收编为 `mutationHandlers.reorderBookmarks` + `commit(..., {save:true})`**（D-002④ 裁定；add 类具名 N-108-01 不同票）；九处非 handler 归属的合法改写落具名注释豁免（conn-layer ×3 / settings-ui ×2 / storage ×2 / sync-engine ×1 / utils ×1）；接入 `package.json` pretest 链第 2 位（八门）+ `.github/workflows/test.yml` 显式步骤（lane = 主 lane `test` job）；新增离线 spec `test/tests/boxing-layout-bypass-gate.spec.ts` 双车道 10/10；**完成门：pretest 八门全绿 + 全量 e2e 639 passed / 5 skipped / 0 failed**；**109R** `test/cluster-map.json` 补 `boxing-pages-gc-version.spec.ts` -> import-graph-guard exit 0；**113R** `docs/release-status.md` 商店 published 对齐 2026.9.15 -> locale-readme-guard exit 0（guard 基线未改，自测仍 PASS）；调研：atomcode 13 源（ESLint unused-disable-directives / Notion ratchet / Immer / Redux / dependency-cruiser / Nx），结论四「能改走 handler 的先改完 + 残余具名豁免」；报告 reports/108-report.md） |
| A-064 | D-001/D-003 · Q3 调研 · B75：G-C 仅三 URL 200，无法发现「过时但 200」；Pages 发行后同步无新鲜度断言 | G-C=200 + GET demo/version.json(cache-buster)==最新 tag；deploy 尾部 verify ≤180s；保留人工检查单；修订 ADR-0017 G-C 行；吸收 B75 | 不做 mike 多版本；不取消人工复核；失败两态写入检查单；核查 environment 放行 tag、Pages 仅 Actions | implemented（2026-09-15 · 票 109：build-demo 产出 version.json={version,deployedAt}+回读校验+demo 渲染版本；scripts/pages-gc-verify.mjs（gate/deploy-tail/self-test）；demo-deploy deploy 尾部 180s verify；ADR-0017 修订 2026-09-15 + publishing-guide + release-status + WORKFLOW 4.4 检查单；B75 关闭；Pages=Actions 单一已核查；environment 未放行 tag -> 需人工放行 v*） |
| A-065 | D-004① · B71 · N-101-06：zoom-dblclick 异源签名（**签名更正 N-110-01**：台账原记 L192「一次 dblclick 双建」；本票实测签名为 **L172-174 enter poll 超时 = 单击未进入 large box**，旧签名 100+ 次运行零复现） | 根因修复或稳定化；禁 skip 冒充绿；回归可复现 | 不弱化该断言不变式；属创建冷却族非 fire-and-forget | implemented（2026-09-15 · ticket 110 · 根因实测：`renderCanvas()` 插入大框后同一帧内 `elementFromPoint(框中心)` 解析到 `.large-box` **根元素**（探针 P3：39/39，且 `kids=7/bodyH=172` 说明子树已挂载），而「点击进框」处理器绑在深层子元素 `.large-box__body`（`ntp/render.js:560`），click 只向上冒泡不向下传播 → 处理器不触发 → `#inner` 不 un-hide → L172-174 15s 轮询超时；探针 P2 对照：稳态 38/38 命中 `.large-box__body`。修复（BX-DEV-112G）：抽出 `onBoxEnterClick`，保留 body 绑定并新增根绑定 `el.addEventListener('click', ev => { if (ev.target === el) onBoxEnterClick(ev); })`——`ev.target === el` 保证稳态不双触发、BX-DEV-112C one-shot 抑制语义不变；BX-DEV-112D 350ms/12px 创建冷却**未改动**（调研结论为保留，缺陷不在该冷却）。验证：新增确定性回归用例 BX-DEV-112G（同一 evaluate 内创建+点击，+37 行）——摘除修复**必失败**、恢复即通过（11.2s，sha256 逐字节还原）；原缺陷用例修复后 **120 连跑产品签名 0 次**（基线 1/40 ≈ 2.5%），唯一非绿为 `browserContext.close` 装置拆卸错误（N-110-02）；全量 `629 passed / 5 skipped / 0 failed`（chromium+firefox 双车道）零回归；零 skip/fixme 新增，断言不变式未弱化；具名 F-110-01（atomcode 通道未暴露，改用 ctx 抓取）、F-110-02（CI run URL 待授权）、N-110-04（旧签名未复现）、N-110-05（test-mutex 参数切分沿用 101 报告 N-101-07）；报告 reports/110-report.md） |
| A-066 | D-004② · B72 · N-106-01：innerclip 两 spec 未设 test.setTimeout 预算（原观察至 2026-10-14，D-004 提前进 9.20） | 为相关 spec 设合理 setTimeout 预算或等价稳定性措施 | 不得再以观察未到期推迟；不入 G-A 豁免台账扩大面 | implemented（2026-09-15 · ticket 111 · 两 spec 各加 test.setTimeout(120_000) + 书面理由注释（票 13 同签名同形）；断言逐字未动（git diff +14/-0）；双车道实测 4/4（firefox 11.3s / chromium 6.9s），争用压力 workers=4 亦 4/4（20.5s，单测 12.0–16.4s；相对 30s 默认仅 1.8–2.5× 余量，预算后 7–10×）；pretest 6 门 5 绿（第 6 门 locale-readme-guard 为既有红 N-111-01，属 113/99 面）；waiver-ledger-check exit 0、台账零改动（不入 G-A 豁免台账）；docs/testing-governance.md 观察行 closed 2026-09-15 + 新增「第三结案路径（预防性修复）」；残余 N-111-02（worker 预热 fixture，建议立票）；报告 reports/111-report.md） |
...
| A-P02 | D-004⑨/D-005③ 豁免清零出口 | 3 条 active F 09-19 到期；按 105 撤账至 0 active F | 绑 9.20 G-A；不得无动作续期 | implemented（2026-09-22 C01：empty-state/search 按 105 双绿撤账 closed；zoom-dblclick expired-handled（票110 已修、105 双绿不足，非续期）；**active F=0**；waiver-ledger-check exit 0） |
| A-P03 | D-006 发行出口 | G-A 新 tip 四 job 全绿 + G-B 用户声明 + G-C 升格；tag/商店另令 | 禁 agent 代签 G-B；9.15 不热修；**G-B≠三门齐** | implemented（2026-09-22 W920 followup：**G-A** run 35694278677 四 job 全绿 · **G-B** 用户声明「人工已经声明通过，执行人 Xxx91n」2026.9.20 · 2026-09-22 · **G-C 升格** verify:pages-gc PASS（version.json=2026.9.15==最新 tag）；**C08 tag/商店仍另令**；对 v2026.9.20 的 G-C 待 tag 后复验。禁止把本行读成「2026.9.20 可发行」直至 C08 + G-C(v2026.9.20) |
