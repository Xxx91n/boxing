# Handoff — Ticket 109: G-C 升格：version.json 新鲜度 + deploy verify

> Covers: A-064 · Blocked by: None  
> Spec: `.scratch/architecture-recovery/spec.md` · Issue: `issues/109-pages-gc-version-verify.md`

## 一句话

发行后 Pages demo version == 最新 tag；部署尾部自动 verify；人工检查单保留。

## 必读

| 用途 | 路径 |
|---|---|
| 本票 | issues/109-pages-gc-version-verify.md |
| Spec | spec.md |
...
3. **对标** 工业级实现/测试策略。  
调研结论写入 `reports/109-report.md` 开头，再进入实现。
...
遵循本 handoff：AC 全勾或具名 F/N 注明；附 CI/实测锚点；A 账本 A-064 状态更新；报告落盘。  
