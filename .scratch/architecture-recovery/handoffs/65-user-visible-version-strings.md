# Handoff — 65 统一用户可见版本串（P2）

## 票面
- Issue: issues/65-user-visible-version-strings.md
- Spec: spec.md（Wave7 节）
- Ledger: decision-ledger.md 覆盖 A-019
- Blocked by: None (can start immediately)

## 完成定义
遵循 handoff 内的完成定义：AC 全勾 + 报告落 reports/65-report.md + 验证命令结果可复核。

## 版本控制
遵循 WORKFLOW §4.2（GitButler but CLI；不 push、不开 PR，除非用户明确要求）。

## 必读
1. issues/65-user-visible-version-strings.md
2. spec.md（Wave7）
3. decision-ledger.md A-019
4. WORKFLOW.md §4.2
5. AGENTS.md
6. manifest.json
7. ntp/ntp.js
...
## 本票 delta
- 主路径 ntp/ntp.js 注释与 debug 串,- 见 issue AC
...
## 完成时
报告写入 reports/65-report.md；但 commit 按 §4.2；不擅自 push/tag/close #9。
