# Handoff — Ticket 108: 静态门禁：layout 旁路不得绕过 mutationHandlers

> Covers: A-063 · Blocked by: 107  
> Spec: `.scratch/architecture-recovery/spec.md` · Issue: `issues/108-layout-bypass-static-gate.md`

## 一句话

pretest/test.yml 能拦下 layout 集合在 handlers 外的删除/改写调用。

## 必读

| 用途 | 路径 |
|---|---|
| 本票 | issues/108-layout-bypass-static-gate.md |
| Spec | spec.md |
...
3. **对标** 工业级实现/测试策略。  
调研结论写入 `reports/108-report.md` 开头，再进入实现。
...
遵循本 handoff：AC 全勾或具名 F/N 注明；附 CI/实测锚点；A 账本 A-063 状态更新；报告落盘。  
