# Handoff — 56 legacyReader 冻结读端升格 rollback AC（P2）

## 票面
- Issue: issues/56-legacy-reader-freeze-rollback-ac.md
- Spec: spec.md（Wave6 节）
- Ledger: decision-ledger.md 覆盖 A-010
- Blocked by: None (can start immediately)

## 完成定义
遵循 issues/56-legacy-reader-freeze-rollback-ac.md 全部验收项 + 报告落盘。

## 版本控制
遵循 WORKFLOW §4.2（GitButler but CLI；不 push、不开 PR，除非用户明确要求）。

## 必读
1. issues/56-legacy-reader-freeze-rollback-ac.md
2. spec.md（Wave6）
...
9. scripts/migration-golden-guard.mjs
10. test/fixtures/schema
...
## 完成时
- 报告: reports/56-legacy-reader-freeze-rollback-ac-report.md
- 报告路径必须写进本窗口最终回复
