# Handoff — Ticket 113: release-status 单一状态块

> Covers: A-068 · Blocked by: None  
> Spec: `.scratch/architecture-recovery/spec.md` · Issue: `issues/113-release-status-single-block.md`

## 一句话

只读 release-status 首屏得到与 tag/商店一致的唯一现役结论。

## 必读

| 用途 | 路径 |
|---|---|
| 本票 | issues/113-release-status-single-block.md |
| Spec | spec.md |
...
3. **对标** 工业级实现/测试策略。  
调研结论写入 `reports/113-report.md` 开头，再进入实现。
...
遵循本 handoff：AC 全勾或具名 F/N 注明；附 CI/实测锚点；A 账本 A-068 状态更新；报告落盘。  
