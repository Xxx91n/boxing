# Handoff — 62 Git 历史声明（条件）（P2）

## 票面
- Issue: issues/62-git-history-declaration.md
- Spec: spec.md（Wave7 节）
- Ledger: decision-ledger.md 覆盖 A-016
- Blocked by: None (can start immediately)

## 完成定义
遵循 handoff 内的完成定义：AC 全勾 + 报告落 reports/62-report.md + 验证命令结果可复核。

## 版本控制
遵循 WORKFLOW §4.2（GitButler but CLI；不 push、不开 PR，除非用户明确要求）。

## 必读
1. issues/62-git-history-declaration.md
2. spec.md（Wave7）
3. decision-ledger.md A-016
4. WORKFLOW.md §4.2
5. AGENTS.md
6. docs/history/
7. README.md
...
## 完成时
报告写入 reports/62-report.md；但 commit 按 §4.2；不擅自 push/tag/close #9。
